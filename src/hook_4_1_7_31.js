...
var sendMessageCallbackFunc = baseAddr.add(0x865DBB8);
var messageCallbackFunc1 = ptr(0);

// 保存原始指令，用于 patch 后还原
var origTextProtobuf = patchTextProtobufAddr.readByteArray(4);
var origTextProtobufDelete = PatchTextProtobufDeleteAddr.readByteArray(4);
var textPatched = false;


// 双方公共使用的地址
var triggerX1Payload = Memory.alloc(0x200);
var triggerX0;
...
// 图片消息全局变量
var sendImgMessageCallbackFunc = baseAddr.add(0x865D680);
...
// 保存图片 patch 原始指令
var origImgProtobufFunc1 = patchImgProtobufFunc1.readByteArray(4);
var origImgProtobufFunc2 = patchImgProtobufFunc2.readByteArray(4);
var origImgProtobufDelete = imgProtobufDeleteAddr.readByteArray(4);
var imgPatched = false;
...
var uploadFunc1 = baseAddr.add(0x802b8b0);
var uploadFunc2Addr = ptr(0)
var uploadFunc2 = baseAddr.add(0x7fd5908);
...
function patchTextProtoBuf() {
    ...
    textPatched = true;
    console.log("[+] Text protobuf patch 已应用");
}

function restoreTextProtoBuf() {
    if (!textPatched) return;
    Memory.patchCode(patchTextProtobufAddr, 4, code => {
        code.writeByteArray(origTextProtobuf);
    });
    Memory.patchCode(PatchTextProtobufDeleteAddr, 4, code => {
        code.writeByteArray(origTextProtobufDelete);
    });
    textPatched = false;
    console.log("[+] Text protobuf patch 已还原");
}
...
function triggerSendTextMessage(taskId, receiver, content, atUser) {
    ...
    if (!triggerX0 || triggerX0.isNull()) {
        console.error("[!] triggerX0 尚未捕获，请先让微信产生一次网络请求（如切换聊天窗口），然后重试");
        return "not_ready";
    }
...
function AttachSendTextProto() {
    Interceptor.attach(sendFuncAddr, {
        onEnter: function (args) {
            ...
            console.log(`[+] 捕获到 StartTask 调用，X0地址：${triggerX0}`);
        }
    })
}
...
function patchImgProtoBuf() {
    ...
    imgPatched = true;
    console.log("[+] Image protobuf patch 已应用");
}

function restoreImgProtoBuf() {
    if (!imgPatched) return;
    Memory.patchCode(patchImgProtobufFunc1, 4, code => {
        code.writeByteArray(origImgProtobufFunc1);
    });
    Memory.patchCode(patchImgProtobufFunc2, 4, code => {
        code.writeByteArray(origImgProtobufFunc2);
    });
    Memory.patchCode(imgProtobufDeleteAddr, 4, code => {
        code.writeByteArray(origImgProtobufDelete);
    });
    imgPatched = false;
    console.log("[+] Image protobuf patch 已还原");
}
    ...
    Interceptor.attach(uploadImageAddr, {
        onEnter: function (args) {
            try {
                ...
                console.log("[-] UploadMedia onEnter 读取异常(不影响正常上传): " + e);
            }
        }
    })
}
...
function setReceiver() {
    ...
    Interceptor.attach(buf2RespAddr, {
        onEnter: function (args) {
            ...
            var messages = [];
            var senderNickname = ""
            var safeUserContent = userContent || ""
            ...
            if (sender.includes("@chatroom")) {
                ...
                // 处理用户的名称
                splitIndex = safeUserContent.indexOf(':')
                if (splitIndex === -1) {
                    splitIndex = safeUserContent.indexOf('在群聊中@了你')
                    senderNickname = splitIndex !== -1 ? safeUserContent.substring(0, splitIndex).trim() : "";
                } else {
                    senderNickname = safeUserContent.substring(0, splitIndex).trim();
                }

            } else {
                // 处理用户的名称
                const splitIndex = safeUserContent.indexOf(':')
                senderNickname = splitIndex !== -1 ? safeUserContent.substring(0, splitIndex).trim() : "";
